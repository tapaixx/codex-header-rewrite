...
|---|---|---|
| `codex-header-rewrite-v0.25.4.so` | `codex-header-rewrite` | `0.25.4` |
...
sudo install -m 0644 codex-header-rewrite-linux-amd64.so \
  /CLIProxyAPI/plugins/codex-header-rewrite-v0.25.4.so
```
...
- **开着**：探针拿到 state 后，再发一次同样的最小请求，**带上这条 state、不带 Cookie**，只测 state 本身，不测会话。复核响应里**没有**新 state，说明这条 state 撑住了，才连同会话入池；回写了新 state 判为降智，不入池；复核请求失败（没响应或非 2xx）算没判定，也不入池。判定和正常请求用的是同一条规则（`judgeInjectedTurn`）。
