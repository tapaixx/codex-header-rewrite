...
- 回合状态（X-Codex-Turn-State）溯源：state 按「凭证 + 模型」持久化入池（降智判定当前已暂停，判定逻辑独立在 `degraded.go`；暂停期间探针自动入池，正常请求在详情里手动入池），发现跨账号回带并可按规则摘除；内置 Fernet 信封解码。
...
**探针按响应判定降智（v0.25.1）。** 探针会把响应读完才决定入不入池，所以它能按响应体判定，而不只看 state。判定用的标记**只写在服务器上的插件配置里**，不在仓库、不在发布的二进制里：
...
plugins:
  configs:
    codex-header-rewrite:
      data_path: "plugins/data/codex-header-rewrite.db"
      probe_degraded_markers: "<标记1>, <标记2>"
```

响应里的事件名、事件 `type` 或字段名包含任一标记（不区分大小写），这次探针就判为降智，state 不入池，探针历史标「疑似降智 · 未入池」；模型写出的正文不参与匹配。没配标记时探针退回 `degraded.go` 的现行判定（目前是暂停，拿到即入池）。`probe_degraded_markers` 和 `data_path` 一样注册成插件配置项，在 CPA 管理端的插件配置里直接填写即可；填一行逗号分隔的字符串，或者存成列表，两种写法插件都认。修改配置后 CPA 下发 reconfigure 即生效，不用重装插件。本插件的面板只显示「按响应判定 / 未判定」，不会读出标记本身，插件的管理接口也不返回它；能看到标记的只有能打开 CPA 插件配置的人。
...
**降智判定当前已暂停（v0.25.0 起）。** 原先按 state 长度判（Team ≤ 332 字符、个人 ≤ 292）的规则已经和上游对不上，所以现在不做判定，拦截、拦截后重试、注入后降智即剔除这三件事因为没有判定结果而都不会触发（开关保留，判定恢复后即生效），历史里的「质量判定」显示「已暂停」。判定逻辑集中在 `degraded.go` 的 `degradedJudge`：长度规则作为 `judgeByLength` 原样保留，恢复或换新规则只改这一处。

**判定暂停期间，正常请求不自动入池（v0.25.1）。** 分不出降智，就不该让客户端自己的流量往池里写：正常请求拿到的 state 只记录来源（跨号回带照常能认出来），历史里标「需手动入池」。确认它可用后，在请求详情的「分析」页点「上游返回」卡片上的「手动入池」：插件用这条记录里保存的响应 state，以及记录里的 Cookie 叠加 Set-Cookie 还原出的会话入池，**不会向上游发任何请求**。手动入池是人工选择，所以即使池里那条更新也会被替换（面板会提示）；State 池维护关闭时不能入池。自动探针（以及拦截后重试）拿到的 state 照常自动入池。判定恢复后，正常请求的合格 state 会重新自动入池。合格 state 每拿到一次就铸造一次，同一「凭证 + 模型」只保留时间最新的一条；乱序到达不会让旧值覆盖新值。
