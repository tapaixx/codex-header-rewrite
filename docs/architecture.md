...
- `judgeProbeVerification`（自动探针，开了 `probe_verify` 时）：探针拿到 state 后再发一次最小请求，带这条 state 和它的会话（Cookie 来源给的 Cookie 叠加响应的 Set-Cookie；「不注入」模式下就是出口下发的那份），按上面同一条规则读复核响应；复核失败不判定。`probe_verify` 关着时探针不判定，走 `degradedJudge`。
