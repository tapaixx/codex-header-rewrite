...
|---|---|---|
| `codex-header-rewrite-v0.25.3.so` | `codex-header-rewrite` | `0.25.3` |
...
sudo install -m 0644 codex-header-rewrite-linux-amd64.so \
  /CLIProxyAPI/plugins/codex-header-rewrite-v0.25.3.so
```
...
**探针按响应耗时判定降智（v0.25.3）。** 探针每次发的都是同一个最小请求，所以它自己的往返时间就是可比的信号：
...
耗时只算**这一次上游调用**：预热请求、读凭证、连代理都不计入。阈值是 `degraded.go` 里的 `probeCleanLatency` / `probeSuspectLatency`，面板从探针接口读同一组数字，不自己抄一份。「可疑」是第三种结果，不是降智：它不入池，但历史里也不会说它降智。
