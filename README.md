...
- 回合状态（X-Codex-Turn-State）溯源：state 按「凭证 + 模型」持久化入池（降智判定当前已暂停，判定逻辑独立在 `degraded.go`），发现跨账号回带并可按规则摘除；内置 Fernet 信封解码。
...
**降智判定当前已暂停（v0.25.0 起）。** 原先按 state 长度判（Team ≤ 332 字符、个人 ≤ 292）的规则已经和上游对不上，所以现在不做判定：上游返回的每个 state 都入池，拦截、拦截后重试、注入后降智即剔除这三件事因为没有判定结果而都不会触发（开关保留，判定恢复后即生效），历史里的「质量判定」显示「已暂停」。判定逻辑集中在 `degraded.go` 的 `degradedJudge`：长度规则作为 `judgeByLength` 原样保留，恢复或换新规则只改这一处。合格 state 每拿到一次就铸造一次，同一「凭证 + 模型」只保留时间最新的一条；乱序到达不会让旧值覆盖新值。
