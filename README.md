...
|---|---|---|
| `codex-header-rewrite-v0.25.12.so` | `codex-header-rewrite` | `0.25.12` |
...
sudo install -m 0644 codex-header-rewrite-linux-amd64.so \
  /CLIProxyAPI/plugins/codex-header-rewrite-v0.25.12.so
```
...
**不降智 Cookie 池（v0.25.8）**：在「请求与管理」里 State 池下面。只收有「不降智」判定的会话 —— 开了多重判断、复核通过的探针会话，以及注入了 state 而上游没有回写的线上会话 —— 按 `__oailb` 里的路由目标去重，同一个路由目标来了新的就覆盖；没有 `__oailb` 的会话不收。列表显示路由目标、签发与过期的本地时间、来源、入池时间；状态由 `__oailb` 的过期时间决定。Cookie 来源选「Cookie 池」时，探针从里面随机取一条**未过期**的带上，池里没有可用的就**不带 Cookie**。**冷却（v0.25.12，取代 v0.25.10 的永久失效）**：Cookie 池卡片上按凭证配置「冷却时间」（秒，留空按 1800）。注入了 Cookie 的线上请求被判为降智（注入的 state 之上上游又回写了一个），或探针从池里取的 Cookie 在多重判断里判为降智，Cookie 池里这个路由目标的那条就冷却这么久 —— 仍在列表里，状态显示「冷却至 HH:MM:SS」，冷却期间探针不会取用；再次降智重新计时，冷却结束后没过期就恢复可用；同一路由目标来了新的不降智 Cookie 就直接覆盖它。历史里对应的请求标「Cookie 冷却」。v0.25.10–v0.25.11 标成「已失效」的条目按失效时刻起算冷却。

**探针高级选项（v0.25.12）**：自动探针卡片底部新增默认折叠的「高级选项」。
- **覆盖请求 Header**：对每一次探针请求生效（探针、多重判断复核、动态代理池的预热），存为规则的 `probe_headers`；`Authorization`、`Cookie`、`X-Codex-Turn-State` 由插件决定，保存时拒绝。
- **手动探针**：选一个探针模型点「立即探针」，按当前探针配置发一次（Cookie 来源、代理、多重判断、覆盖 Header 都一样），不看「启用自动探针」开关、生效时段和活跃判定；State 池维护关闭时拒绝。请求在后台跑（`POST /codex-header-rewrite/probe/run`），结果进探针历史并标「手动」，入池、Cookie 池和冷却的处理与自动触发的相同。

**请求路由与响应路由（v0.25.12）**：请求历史和探针历史各有两列 ——「请求路由」是发出的 Cookie 里 `__oailb` 的路由目标，「响应路由」是响应 Set-Cookie 里 `__oailb` 的路由目标；两者不同说明负载均衡把会话换到了另一个后端，响应没有下发 `__oailb` 时显示「-」。

**手动入池与替换凭证级 Cookie（v0.25.11）**：
- Cookie 池卡片标题栏的「添加」可以粘贴一份完整 Cookie 入池；请求详情「分析」页 Cookie 一节的「Cookie 入池」把这次请求的会话（发出的 Cookie 叠加响应的 Set-Cookie）入池。两者都必须带 `__oailb`，按路由目标覆盖，来源记为「手动」。
- 点开 Cookie 池里的一条，「替换为凭证级 Cookie」把它写进这个凭证的凭证级 Cookie（「凭证级」Cookie 来源的探针带的那份），需要确认；最近真实请求时间不变。这个凭证的下一次真实请求会再覆盖它。线上会话要真正代表上游看到的 Cookie，需要开着「注入 Cookie」的转发（CPA 默认不转发 Cookie）。
