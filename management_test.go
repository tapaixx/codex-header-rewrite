...
func TestManagementCredentialFilterAndRule(t *testing.T) {
	...
	var list struct { Credentials []credentialView `json:"credentials"` }
	if err := json.Unmarshal(resp.Body, &list); err != nil { t.Fatal(err) }
	if len(list.Credentials) != 1 || list.Credentials[0].AuthIndex != "idx-a" { t.Fatalf("creds=%#v", list.Credentials) }
	...
	if resp.StatusCode != 200 { t.Fatalf("put=%d %s", resp.StatusCode, resp.Body) }
	resp, _ = handleManagementAPI(managementRequest{Method: "GET", Path: "/v0/management" + apiRulePath, Query: url.Values{"auth_index": {"idx-a"}}})
	var got struct { Rule headerRule `json:"rule"` }
	_ = json.Unmarshal(resp.Body, &got)
	if got.Rule.Set["X-Test"] != "v" { t.Fatalf("rule=%#v", got.Rule) }
}
