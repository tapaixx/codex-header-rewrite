...
- `judgeProbeVerification`（自动探针，开了 `probe_verify` 时）：探针首个请求不带 Cookie；拿到 state 后再发一次最小请求，带这条 state 和同一响应下发的 Cookie，按上面同一条规则读复核响应；复核失败不判定。`probe_verify` 关着时探针不判定，走 `degradedJudge`。
