...
- **开着**：探针拿到 state 后，再发一次同样的最小请求，**带上这条 state 和同一响应下发的 Cookie**（v0.25.7 起；v0.25.4–v0.25.6 不带 Cookie）。复核响应里**没有**新 state，说明这条 state 撑住了，才连同会话入池；回写了新 state 判为降智，不入池；复核请求失败（没响应或非 2xx）算没判定，也不入池。判定和正常请求用的是同一条规则（`judgeInjectedTurn`）。
...
**探针不带 Cookie（v0.25.7）。** Cloudflare 的 `__cf_bm`、负载均衡的 `__oailb` 都绑定拿到它的那个出口 IP，存下来的 cookie 换个出口就是错的。所以探针第一次请求**不带任何 Cookie**，这个出口回给它的 state 和 Set-Cookie 就是一对：

- 多重判断关着：state 连同这次响应下发的 cookie 一起入池。
- 多重判断开着：复核请求带上这条 state **和这次响应下发的 cookie**，从同一条连接（同一个出口）发出；上游没有回写新 state 才入池，入池的会话是复核响应更新后的 cookie。

原来的三种 Cookie 来源（凭证级 / 静态代理 / 动态代理池）和动态代理池的预热请求都去掉了，探针也不再维护自己的 cookie 罐；旧版写下的探针历史照常显示「Cookie 来源 · 旧版」。

**凭证级 Cookie** 仍由这个凭证的真实请求维护（请求带的 Cookie 叠加响应的 Set-Cookie），在「请求与管理 › State 池」卡片上点「凭证级 Cookie」可以看原值和解析卡片，以及最近更新时间和最近真实请求时间。
