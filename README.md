...
|---|---|---|
| `codex-header-rewrite-v0.25.0.so` | `codex-header-rewrite` | `0.25.0` |
...
sudo install -m 0644 codex-header-rewrite-linux-amd64.so \
  /CLIProxyAPI/plugins/codex-header-rewrite-v0.25.0.so
```
...
响应里的事件名、事件 `type` 或字段名包含任一标记（不区分大小写），这次探针就判为降智，state 不入池，探针历史标「疑似降智 · 未入池」；模型写出的正文不参与匹配。没配标记时探针退回 `degraded.go` 的现行判定（目前是暂停，拿到即入池）。修改配置后 CPA 下发 reconfigure 即生效，不用重装插件。面板只显示「按响应判定 / 未判定」，不会读出标记本身，管理接口也不返回它。
